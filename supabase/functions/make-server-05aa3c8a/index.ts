...
import * as kvRetry from "./kv_retry.ts"; // Retry-обёртки для надёжной работы с KV
import { createClient } from "npm:@supabase/supabase-js@2";
import { getUserRank, invalidateRankCache, updateUplineRanks, updateUserRank, calculateUserRank } from "./rank_calculator.ts";
import * as metricsCache from "./user_metrics_cache.ts";
...
app.post("/make-server-05aa3c8a/auth/login", async (c) => {
  try {
    ...
    // 🆕 Check if this is a KV-only user (registered when Supabase Auth was unavailable)
    let authData: any = null;
    let authError: any = null;
    
    if (userData.isKvOnlyUser && userData.passwordHash) {
      console.log(`🔐 KV-only user detected: ${userData.id}, verifying password hash...`);
      
      // Verify password using stored hash
      const providedHash = await createHmacSha256(password, userEmail.toLowerCase());
      
      if (providedHash === userData.passwordHash) {
        console.log(`✅ KV-only auth successful for ${userData.id}`);
        authData = {
          session: {
            access_token: `kv_token_${userData.id}_${Date.now()}`,
            refresh_token: `kv_refresh_${userData.id}_${Date.now()}`
          },
          user: {
        ...
        console.log(`❌ KV-only auth failed: incorrect password for ${userData.id}`);
...
app.post("/make-server-05aa3c8a/register", async (c) => {
  try {
    ...
    // Try to create user in Supabase Auth, but continue with KV-only if it fails
    console.log('Creating user in Supabase Auth via signUp...');
    ...
    try {
      ...
      if (signUpError) {
        console.log(`Supabase signUp error: ${signUpError.message}`, signUpError);
        console.log('⚠️ Continuing with KV-only registration (Supabase Auth unavailable)');
    ...
    // Hash password for KV-only users (simple hash for fallback auth)
    const isKvOnlyUser = authUserId.startsWith('kv_');
    const passwordHash = isKvOnlyUser ? await createHmacSha256(password, email.trim().toLowerCase()) : null;
...
app.post("/make-server-05aa3c8a/auth/signup-admin", async (c) => {
  try {
    ...
    const { getPermissionsForRole } = await import('./admin_helpers.ts');
...
// OPTIONS handler for /admin/delete-user (CORS preflight for DELETE)
app.options("/make-server-05aa3c8a/admin/delete-user/:userId", (c) => {
  console.log('🗑️ OPTIONS /admin/delete-user - CORS preflight');
  return c.text('', 204, {
    'Access-Control-Allow-Origin': '*',
